...
{
	DP_LOG_FUNC();
	drm_svc_init_roap_queue();
}

bool DownloadUtil::getMimeFromDrmContent(const char *path, string& out_mimeType)
{
	int typeLen = 0;
	...
	memset(&contentInfo, 0x00, sizeof(drm_content_info_t));

	drm_svc_get_content_info(path, &contentInfo);

	typeLen = strlen(contentInfo.contentType);
	if (typeLen < 1 ) {
		DP_LOGE("there is no mime type");
		return false;
	}

	out_mimeType.assign(contentInfo.contentType);
	return true;
}


bool DownloadUtil::checkJavaDRMContentType(const char *filePath)
{
	DP_LOGD_FUNC();

	if (filePath == NULL) {
		DP_LOGE("Param is NULL");
		return false;
	}

	string derivedMime;
	if (getMimeFromDrmContent(filePath,derivedMime)) {
		if (derivedMime.length() > 0) {
			DP_LOG("mime from drm content : [%s]",derivedMime.c_str());
			if (getContentType(derivedMime.c_str(), filePath) == DP_CONTENT_JAVA) {
				DP_LOG("MIDP play type");
				return true;
			}
		}
	}
