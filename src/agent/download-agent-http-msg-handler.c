 ...
 *
 * @file        download-agent-http-msg-handler.c
 * @brief       Utilities to manipulate HTTP messages
...
void __http_header_destroy_all_field(http_header_t **head)
{
	...
	while (cur) {
		if (cur->field) {
			DA_LOG(HTTPManager, "field= %s", cur->field);
			free(cur->field);
			cur->field = DA_NULL;
		}
	...
	const char *in_field, http_header_t **out_header)
{
	...
	while (http_msg_get_header_with_iter(&http_msg_iter, &field, &header)) {
		if (field && header && !strncmp(field, in_field, strlen(field))) {
			DA_LOG(HTTPManager, "[%s][%s]", field, header->value);
			*out_header = header;
			return DA_TRUE;
		}
	}
...
da_bool_t http_msg_response_get_content_length(
	http_msg_response_t *http_msg_response, int *out_length)
{
	da_bool_t b_ret = DA_FALSE;
	http_header_t *header = NULL;
